...
class ACMBase {
  ...
  __get_host() {
    ...
    const index = helper.random(list.length);
    return list[index];
  }
  ...
  async __get_signature(request, decode) {
    ...
    headers['spas-accesskey'] = credential.accessKeyId;
    const signature = crypto.createHmac('sha1', credential.accessKeySecret)
